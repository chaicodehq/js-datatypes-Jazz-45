...
export function analyzeUPITransactions(transactions) {
  ...
  const freqMap=validTransactions.reduce((acc,txn)=>{
  ...
  validTransactions.forEach(txn=>{
    if(freqMap[txn.to]>maxCount){
      maxCount=freqMap[txn.to];
      frequentContact=txn.to;
    }
  });

  const allAbove100=
    validTransactions.every(txn=>txn.amount>100);

  const hasLargeTransaction=
    validTransactions.some(txn=>txn.amount>=5000);
